...
class TestCLI(unittest.TestCase):
    ...
    @patch('hyprrice.cli.migrate_user_config')
    def test_cmd_doctor_autofix_success(self, mock_migrate, mock_mkdir, mock_exists, mock_check_deps):
        """Test doctor command auto-fix functionality with success."""
        ...
        # Mock filesystem - config and directories don't exist
        ...
        with patch('builtins.print') as mock_print:
            result = cmd_doctor(args)
            self.assertEqual(result, 1)  # Should return 1 due to dependency issues
            ...
            self.assertTrue(any("Configuration created" in call for call in print_calls))
            self.assertTrue(any("Plugins directory created" in call for call in print_calls))
            self.assertTrue(any("Backup directory created" in call for call in print_calls))
    
    @patch('hyprrice.cli.check_dependencies')
    @patch('pathlib.Path.exists')
    @patch('pathlib.Path.mkdir')
    @patch('hyprrice.cli.migrate_user_config')
    def test_cmd_doctor_autofix_partial_failure(self, mock_migrate, mock_mkdir, mock_exists, mock_check_deps):
        """Test doctor command auto-fix with partial failures."""
        # Mock dependencies with some failures
        mock_check_deps.return_value = {
            'hyprland': {'available': False},
            'waybar': {'available': True},
        ...
        # Mock filesystem - config doesn't exist, directories do
        def exists_side_effect(path):
            if 'config.yaml' in str(path):
                return False
            return True
        ...
        with patch('builtins.print') as mock_print:
            ...
            print_calls = [call[0][0] for call in mock_print.call_args_list]
            self.assertTrue(any("Failed to create config" in call for call in print_calls))
    
    @patch('hyprrice.cli.check_dependencies')
    @patch('pathlib.Path.exists')
    def test_cmd_doctor_autofix_no_issues(self, mock_exists, mock_check_deps):
        """Test doctor command auto-fix when no issues are found."""
        ...
        with patch('builtins.print') as mock_print:
            ...
            # Verify no auto-fix messages were printed
            print_calls = [call[0][0] for call in mock_print.call_args_list]
            self.assertFalse(any("Fixed" in call for call in print_calls))
            self.assertTrue(any("All checks passed" in call for call in print_calls))
    
    @patch('hyprrice.cli.migrate_user_config')
    def test_cmd_migrate_success_with_backup(self, mock_migrate):
        """Test migrate command success with backup creation."""
        mock_migrate.return_value = True
        ...
        with patch('hyprrice.cli.check_migration_needed', return_value=True):
            with patch('builtins.print') as mock_print:
                ...
                print_calls = [call[0][0] for call in mock_print.call_args_list]
                self.assertTrue(any("Migration completed successfully" in call for call in print_calls))
                self.assertTrue(any("Backup created automatically" in call for call in print_calls))
    
    @patch('hyprrice.cli.migrate_user_config')
    def test_cmd_migrate_no_migration_needed(self, mock_migrate):
        """Test migrate command when no migration is needed."""
        args = MagicMock()
        args.backup = True
        args.force = False
        
        with patch('hyprrice.cli.check_migration_needed', return_value=False):
            with patch('builtins.print') as mock_print:
                result = cmd_migrate(args)
                self.assertEqual(result, 0)
                ...
                print_calls = [call[0][0] for call in mock_print.call_args_list]
                self.assertTrue(any("No migration needed" in call for call in print_calls))
                
                # Verify migrate_user_config was not called
                mock_migrate.assert_not_called()
    
    @patch('hyprrice.cli.migrate_user_config')
    def test_cmd_migrate_force(self, mock_migrate):
        """Test migrate command with force flag."""
        mock_migrate.return_value = True
        ...
        with patch('builtins.print') as mock_print:
            ...
            # Verify migration was called despite check_migration_needed
            mock_migrate.assert_called_once()
